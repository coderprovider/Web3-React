...
const isSwitchEthereumChainRequest = (
  x: RequestArguments
): x is SwitchEthereumChainRequestArguments => {
...
class MockWalletConnectProvider extends MockEIP1193Provider<number> {
  ...
  public eth_switchEthereumChain = jest.fn(
    (
      /* eslint-disable-line @typescript-eslint/no-unused-vars */ _args: string
    ) => null
  );

  public request(
    x: RequestArguments | SwitchEthereumChainRequestArguments
  ): Promise<unknown> {
  ...
  get session() {
    return this.eth_requestAccounts.mock.calls.length > 0
      ? {
          ...
          namespaces: {
            eip155: {
              accounts: this.getConnectedChains().map(
                (chainId) => `eip155:${chainId}:0x1`
              ),
            },
          },
        }
...
describe("WalletConnect", () => {
  let wc2InitMock: jest.SpyInstance<
    ReturnType<typeof EthereumProvider.init>,
    Parameters<typeof EthereumProvider.init>
  >;
  ...
  beforeEach(() => {
    ...
    wc2InitMock = jest
      ...
      .mockImplementation((opts) =>
        Promise.resolve(new MockWalletConnectProvider(opts))
      );
  });
  ...
  describe("#activate", () => {
    ...
    test("should throw an error when using optional chain as default", () => {
      expect(() =>
        createTestEnvironment({ chains, optionalChains: [8] }, 8)
      ).toThrow(
    ...
    test("should throw an error when activating an inactive optional chain", async () => {
      jest
        .spyOn(MockWalletConnectProvider.prototype, "getConnectedChains")
    ...
    test("should not switch chain if already connected", async () => {
      ...
      await connector.activate(2);
      expect(
        (connector.provider as unknown as MockWalletConnectProvider)
